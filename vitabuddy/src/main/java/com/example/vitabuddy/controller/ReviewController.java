...
public class ReviewController {
	...
	public String insertReview(@PathVariable("supId") int supId, @RequestParam("reviewTitle") String reviewTitle,
			@RequestParam("rating") String rating, @RequestParam("reviewHashtag") String reviewHashtag,
	...
	public String deleteReview(@PathVariable("supId") int supId, @PathVariable("reviewNo") String reviewNo,
			HttpSession session) {
	...
	public String updateReview(@PathVariable("supId") int supId, @PathVariable("reviewNo") String reviewNo,
			@RequestParam("reviewTitle") String reviewTitle, @RequestParam("rating") String rating,
		...
		String userId = (String) session.getAttribute("sid");
		if (userId == null) {
			return "redirect:/"; // 비회원일 경우 리다이렉트
		}

		ReviewVO reviewVO = new ReviewVO();
		reviewVO.setReviewNo(reviewNo); // 기존 리뷰 번호 사용
		reviewVO.setSupId(supId);
		reviewVO.setUserId(userId);
		reviewVO.setReviewDate(new Date());
		reviewVO.setReviewTitle(reviewTitle);
			...
			for (int i = 0; i < Math.min(3, reviewImgFiles.size()); i++) {
				...
				if (!file.isEmpty()) {
					...
					try {
						...
						e.printStackTrace();
						return "error/fileUploadError"; // 파일 저장 실패
					}
				}
			}
		}
		...
		if (result == 0) {
			return "error/fileUploadError"; // 실패 시 에러 페이지 이동
		}
	...
	public String showEditReviewPage(@PathVariable("supId") int supId, @PathVariable("reviewNo") String reviewNo,
			Model model) {
